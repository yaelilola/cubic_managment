...
class CustomUserTestCase(TestCase):
    def setUp(self):
        ...
        CustomUser.objects.create_user(email='user1@test_group_1.com', password='pass',employee_number='2',percentage='part_time',business_group=bg1)
    ...
    def test_mycubic_one_assignment(self):
        ...
        self.assertEqual(response.status_code, 200)
        user = CustomUser.objects.get(email='user1@test_group_1.com')
        cubic = Cubic.objects.get(id="cubic_test_1")
        assignment_data = {'users': [user], 'cubics': [cubic]}
        response = c.post(reverse('focal_point:assign_part_time'), assignment_data)
        user = CustomUser.objects.get(email='user1@test_group_1.com')
        assignments = AssignUserCubic.objects.get(assigned_user=user)
        self.assertEqual(len(assignments), 1)
        self.assertEqual(assignments[0].cubic, Cubic.objects.get(id="cubic_test_1"))
        self.assertEqual(response.status_code, 200)
